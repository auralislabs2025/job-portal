<x-app-layout>
    <x-slot name="header">
        <div class="page-header">
            <div>
                <div class="breadcrumb"><a href="{{ route('dashboard') }}">Home</a> <span>/</span> <a href="{{ route('jobs.index') }}">Jobs</a> <span>/</span> <span>Create Job</span></div>
                <h2>Create New Job</h2>
                <p>Post a new job opening for a group company under ABN Corporation.</p>
            </div>
        </div>
    </x-slot>

    <div class="card" style="max-width:800px;">
        <form method="POST" action="{{ route('jobs.store') }}" id="createJobForm">
            @csrf
            <div class="grid-2">
                <div class="form-group">
                    <label>Job Title *</label>
                    <input type="text" name="title" class="form-control" required placeholder="e.g. Senior Fleet Manager" value="{{ old('title') }}">
                ...
                <div class="form-group">
                    ...
                    <select name="group_company_id" class="form-control" required>
                        ...
                        @foreach ($companies as $company)
                            <option value="{{ $company->id }}" {{ old('group_company_id') == $company->id ? 'selected' : '' }}>{{ $company->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="location" class="form-control" required placeholder="e.g. Dubai, UAE" value="{{ old('location') }}">
                    @error('location')<span style="color:var(--danger);font-size:0.8rem;">{{ $message }}</span>@enderror
                </div>
                <div class="form-group">
                    <label>Salary Range</label>
                    <input type="text" name="salary" class="form-control" placeholder="e.g. AED 15,000 - 25,000" value="{{ old('salary') }}">
                </div>
                <div class="form-group">
                ...
                <div class="multi-select" id="notificationMultiSelect">
                    ...
                    <div class="multi-select-dropdown" id="multiSelectDropdown">
                        @foreach ($notificationGroups as $ng)
                            <label>
                                <input type="checkbox" name="notification_group_ids[]" value="{{ $ng->id }}" data-label="{{ $ng->name }} ({{ $ng->groupCompany?->name ?? 'All' }})" {{ in_array($ng->id, old('notification_group_ids', [])) ? 'checked' : '' }} onchange="updateMultiSelect()">
            ...
            <div class="flex-between" style="gap:0.7rem;">
                <a href="{{ route('jobs.index') }}" class="btn btn-outline"><i class="fa-solid fa-arrow-left"></i> Cancel</a>
                <div style="display:flex;gap:0.7rem;">
                    <button type="submit" name="action" value="draft" class="btn btn-outline"><i class="fa-solid fa-save"></i> Save as Draft</button>
